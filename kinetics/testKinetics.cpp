...
void testPoolVolumeScaling()
{
    ...
    ObjId mid = shell->doAddMsg("OneToOne", ObjId(poolId, 0), "requestVolume",
...
void testReacVolumeScaling()
{
    ...
    ObjId mid = shell->doAddMsg("OneToOne", subId, "requestVolume", meshId,
                                "get_volume");
    assert(mid != ObjId());
    mid = shell->doAddMsg("OneToOne", prdId, "requestVolume", meshId,
                          "get_volume");
    assert(mid != ObjId());
...
void testTwoReacGetNeighbors()
{
    ...
    vector<Id> pools;
    unsigned int num = reacId.element()->getNeighbors(
        pools, Reac::initCinfo()->findFinfo("toSub"));
    assert(num == 2);
