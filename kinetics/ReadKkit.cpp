...
{
    string ret = input;
    for (unsigned int i = 0; i < input.length(); ++i) ret[i] = tolower(ret[i]);
    return ret;
}
...
Id makeStandardElements(Id pa, const string& modelname)
{
    ...
    if (kinetics == Id()) {
        ...
        SetGet2<double, unsigned int>::set(kinetics, "buildDefaultMesh", 1e-15,
                                           1);
    ...
    assert(graphs != Id());
    Id moregraphs =
        shell->doCreate("Neutral", mgr, "moregraphs", 1, MooseGlobal);
               ...
               const string& method)
{
    ...
    assert(compt != Id());
    string simpath2 = mgr.path() + "/##[ISA=StimulusTable]," + mgr.path() +
                      "/##[ISA=PulseGen]";
    ...
    {
        cout << " Warning:  Default solver set is Exponential Euler. To set
    \'gsl\' or \'gssa\' solver use function
    mooseaddChemSolver(modelpath,\'solverType\')"<<"\n";
    }
 ...
 */
Id ReadKkit::read(const string& filename, const string& modelname, Id pa,
...
void ReadKkit::dumpPlots(const string& filename)
{
    ...
    vector<ObjId> plots;
    string plotpath = basePath_ + "/graphs/##[TYPE=Table2]," + basePath_ +
                      "/moregraphs/##[TYPE=Table2]";
    wildcardFind(plotpath, plots);
    for (vector<ObjId>::iterator i = plots.begin(); i != plots.end(); ++i)
        SetGet2<string, string>::set(*i, "xplot", filename,
                                     i->element()->getName());
}
...
{
    for (unsigned int i = 2; i != args.size(); ++i) argConv[args[i]] = i + 2;
}
...
void ReadKkit::call(const vector<string>& args)
{
    ...
    if (args.size() > 3) {
        ...
        if ((args[1].substr(len - 5) == "notes") && args[2] == "LOAD") {
            if (args[3].length() == 0)
                return;
...
{
    static const Finfo* subFinfo = ReacBase::initCinfo()->findFinfo("subOut");
    assert(subFinfo);
...
{
    for (map<string, Id>::iterator i = reacIds_.begin(); i != reacIds_.end();
         ++i) {
...
{
    static const Finfo* enzFinfo = EnzBase::initCinfo()->findFinfo("enzOut");
    assert(enzFinfo);
...
Id ReadKkit::buildEnz(const vector<string>& args)
{
    ...
    if (isMM) {
        ...
        separateVols(cplx, -1);  // -1 vol is a flag to defer mesh assignment
                                 // for the cplx pool.

        ObjId ret = shell_->doAddMsg("OneToAll", ObjId(enz, 0), "cplx",
                                     ObjId(cplx, 0), "reac");
        assert(ret != ObjId());
...
Id ReadKkit::buildInfo(Id parent, map<string, int>& m,
                       const vector<string>& args)
{
    ...
    Field<string>::set(info, "textColor", args[m["xtree_textfg_req"]]);
    return info;
}
...
Id ReadKkit::buildPool(const vector<string>& args)
{
     ...
     */
    double vol =
        1.0e3 * vsf / KKIT_NA;  // Converts volscale to actual vol in m^3
...
Id ReadKkit::findSumTotSrc(const string& src)
{
    ...
    cout << "Error: ReadKkit::findSumTotSrc: Cannot find source pool '" << src
         << endl;
    assert(0);
    return Id();
}
...
void ReadKkit::buildSumTotal(const string& src, const string& dest)
{
    ...
    if (destId.element()->cinfo()->name() == "Pool") {
        ...
        ObjId ret = shell_->doAddMsg("single", ObjId(sumId, 0), "valueOut",
                                     ObjId(destId, 0), "setN");
        assert(ret != ObjId());
    } else {
    ...
    ObjId ret =
        shell_->doAddMsg("single", ObjId(srcId, 0), "nOut", xi, "input");
    assert(ret != ObjId());

    stringstream ss;
    for (unsigned int i = 0; i < numVars; ++i) ss << "x" << i << "+";
    ss << "x" << numVars;
...
unsigned int ReadKkit::loadTab(const vector<string>& args)
{
    ...
    // cout << "Loading table for " << args[0] << "," << args[1] << "," << clean
    // << endl;
...
void ReadKkit::innerAddMsg(const string& src, const map<string, Id>& m1,
                           const string& srcMsg, const string& dest,
                           const map<string, Id>& m2, const string& destMsg,
                           bool isBackward)
{
    ...
    if (isBackward) {
        ObjId ret = shell_->doAddMsg("AllToOne", ObjId(srcId, 0), srcMsg,
                                     ObjId(destId, 0), destMsg);
        assert(ret != ObjId());
    } else {
        ObjId ret = shell_->doAddMsg("OneToAll", ObjId(srcId, 0), srcMsg,
                                     ObjId(destId, 0), destMsg);
        assert(ret != ObjId());
    }
}
...
void ReadKkit::addmsg(const vector<string>& args)
{
    ...
    } else if (args[3] == "PLOT")  // Time-course output for pool
    {
        ...
        if (i == poolIds_.end()) {
            ...
            if (i == enzIds_.end()) {
                cout << "Error: ReadKkit: Unable to find src for plot: " << src
                     << ", " << dest << endl;
                assert(0);
                return;
            }
            vector<Id> enzcplx;
            i->second.element()->getNeighbors(
                enzcplx, i->second.element()->cinfo()->findFinfo("cplxOut"));
        ...
        if (args[4] == "Co" || args[4] == "CoComplex") {
            ObjId ret =
                shell_->doAddMsg("Single", plot, "requestOut", pool, "getConc");
            assert(ret != ObjId());
        } else if (args[4] == "n" || args[4] == "nComplex") {
            ObjId ret =
                shell_->doAddMsg("Single", plot, "requestOut", pool, "getN");
            assert(ret != ObjId());
        } else {
...
void ReadKkit::setupSlaveMsg(const string& src, const string& dest)
{
    ...
    if (srcId.element()->cinfo()->isA("TableBase")) {
        ...
        cout << "Error: Unknown source for SLAVE msg: (" << src << ", " << dest
             << ")\n";
        return;
    }
    ...
    if (i == poolFlags_.end() || !(i->second & 2)) {
        innerAddMsg(src, *nameMap, output, dest, poolIds_, "setNInit");
    } else {
        innerAddMsg(src, *nameMap, output, dest, poolIds_, "setConcInit");

        double CONCSCALE = 0.001;
        // Rescale from uM to millimolar.
        if (nameMap == &tabIds_) {
            SetGet2<double, double>::set(srcId, "linearTransform", CONCSCALE,
                                         0);
...
void ReadKkit::convertPoolAmountToConcUnits()
{
    ...
    for (map<string, Id>::iterator i = poolIds_.begin(); i != poolIds_.end();
         ++i) {
        Id pool = i->second;
...
void ReadKkit::convertReacRatesToConcUnits()
{
    ...
    for (map<string, Id>::iterator i = reacIds_.begin(); i != reacIds_.end();
        ...
        unsigned int numSub = Field<unsigned int>::get(reac, "numSubstrates");
        unsigned int numPrd = Field<unsigned int>::get(reac, "numProducts");
...
{
    const double NA_RATIO = KKIT_NA / NA;
    for (map<string, Id>::iterator i = mmEnzIds_.begin(); i != mmEnzIds_.end();
         ++i) {
        Id enz = i->second;
        ...
        // NA for kkit is not accurate. So we correct for this.
        double numSub = Field<unsigned int>::get(enz, "numSubstrates");
...
{
    const double NA_RATIO = KKIT_NA / NA;
    for (map<string, Id>::iterator i = enzIds_.begin(); i != enzIds_.end();
         ++i) {
        Id enz = i->second;
