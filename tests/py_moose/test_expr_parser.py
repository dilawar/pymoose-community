...
def makeBis(args=None):
    params = {
        ...
        'diffusionLength':
        1.0e-6,  # Diffusion characteristic length, used as voxel length too.
...
def makeFHN(args=None):
    ...
    makeChemProto('fhn', '5.0*(A - kxa - (A-kxa)^3/3 - (B-kxb) + Ca)',
                  '(A-kxa + k_a - k_b*(B-kxb))/k_t', params)
...
def makeNegFB(args=None):
    ...
    makeChemProto('negFB', 'k1a*A + k2a*A*B + k3a*Ca', 'k1b*A + k2b*B', params)
    return params
...
def makeNegFF(args=None):
    params = {
        ...
        'stimWidth': 1.0,  # Stimulus width in seconds
        'stimAmplitude':
        10.0,  # Stimulus amplitude, arb units. From FHN review
        'blankVoxelsAtEnd': 10,  # of voxels to leave blank at end of cylinder
        'preStimTime': 10.0,  # Time to run before turning on stimulus.
        'postStimTime': 40.0,  # Time to run after stimulus. ~3x decay time
        'settleTime': 20.0,  # Settling time of response, after stimulus.
    ...
    makeChemProto('negFF', 'k1a*A + k2a*A*B + k3a*Ca/(1+k4a*B*B)',
                  'k1b*Ca + k2b*B', params)
