...
int HSolveUtils::adjacent(Id compartment, Id exclude, vector<Id>& ret)
{
    ...
    ret.erase(remove(ret.begin(), ret.end(), exclude), ret.end());
 ...
 */
int HSolveUtils::gates(Id channel, vector<Id>& ret, bool getOriginals)
{
    //        dump("HSolveUtils::gates() is not tested with new hsolve api", "FIXME");
    unsigned int oldSize = ret.size();

    static string gateName[] = {
        string("gateX[0]"), string("gateY[0]"), string("gateZ[0]")};

    static string powerField[] = {string("Xpower"), string("Ypower"), string("Zpower")};
    ...
    for(unsigned int i = 0; i < nGates; i++) {
        ...
        if(power > 0.0) {
            ...
            string gatePath = moose::fixPath(channel.path()) + "/" + gateName[i];
            Id gate(gatePath);
...
void HSolveUtils::rates(
    Id gateId, HSolveUtils::Grid grid, vector<double>& A, vector<double>& B)
{
...
int HSolveUtils::targets(Id object, string msg, vector<Id>& target,
    ...
    bool include)                  // Default: true
{
    ...
    if(filter.empty()) {
        ...
        for(ia = all.begin(); ia != all.end(); ++ia) {
            ...
            bool hit = find(filter.begin(), filter.end(), className) != filter.end();
...
void testHSolveUtils()
{
    ...
    // c0 is adjacent
    success = find(found.begin(), found.end(), c[0]) != found.end();
    ASSERT(success, "Finding adjacent compartments");
    // c2 - c5 are adjacent
    for(int i = 2; i < 6; i++) {
        success = find(found.begin(), found.end(), c[i]) != found.end();
        ASSERT(success, "Finding adjacent compartments");
    }
