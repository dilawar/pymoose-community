...
static double* get_next_work_item()
{
    ...
    for (unsigned int i = 0; i < numEntries; ++i) ret[i] = i + numCalls;
...
static void slave(void)
{
    ...
    while (1) {
        ...
        MPI_Recv(work, numEntries, MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
                 &status);
...
{
    double tot = 0;
    for (int i = 0; i < numEntries; ++i) tot += work[i];
}
