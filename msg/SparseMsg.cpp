...
const Cinfo* SparseMsg::initCinfo()
{
    ...
    static ValueFinfo<SparseMsg, vector<unsigned int>> connectionList(
        ...
        &SparseMsg::setEntryPairs, &SparseMsg::getEntryPairs);

    /// Connection matrix entries to manipulate in Python.
    static ReadOnlyValueFinfo<SparseMsg, vector<unsigned int>> matrixEntry(
        "matrixEntry",
    ...
    static ReadOnlyValueFinfo<SparseMsg, vector<unsigned int>> columnIndex(
        "columnIndex", "Column Index of each matrix entry",
        &SparseMsg::getColIndex);
    /// connection matrix rowStart to manipulate in Python.
    static ReadOnlyValueFinfo<SparseMsg, vector<unsigned int>> rowStart(
    ...
    static ValueFinfo<SparseMsg, double> probability(
        "probability", "connection probability for random connectivity.",
        &SparseMsg::setProbability, &SparseMsg::getProbability);

    static ValueFinfo<SparseMsg, int> seed(
        "seed", "Random number seed for generating probabilistic connectivity.",
        &SparseMsg::setSeed, &SparseMsg::getSeed);

    ////////////////////////////////////////////////////////////////////////
    // DestFinfos
    ////////////////////////////////////////////////////////////////////////

    static DestFinfo setRandomConnectivity(
        "setRandomConnectivity",
        "Assigns connectivity with specified probability and seed",
        new OpFunc2<SparseMsg, double, unsigned int>(
            &SparseMsg::setRandomConnectivity));

    static DestFinfo setEntry(
        "setEntry", "Assigns single row,column value",
        new OpFunc3<SparseMsg, unsigned int, unsigned int, unsigned int>(
            &SparseMsg::setEntry));

    static DestFinfo unsetEntry(
        "unsetEntry", "Clears single row,column entry",
        new OpFunc2<SparseMsg, unsigned int, unsigned int>(
            &SparseMsg::unsetEntry));

    static DestFinfo clear("clear", "Clears out the entire matrix",
                           new OpFunc0<SparseMsg>(&SparseMsg::clear));

    static DestFinfo transpose("transpose", "Transposes the sparse matrix",
                               new OpFunc0<SparseMsg>(&SparseMsg::transpose));

    static DestFinfo pairFill(
        "pairFill",
        ...
        new OpFunc2<SparseMsg, vector<unsigned int>, vector<unsigned int>>(
            &SparseMsg::pairFill));

    static DestFinfo tripletFill(
        "tripletFill",
        "Fills entire matrix using triplets of (x,y,fieldIndex) to fully "
        "specify every connection in the sparse matrix.",
        new OpFunc3<SparseMsg, vector<unsigned int>, vector<unsigned int>,
                    vector<unsigned int>>(&SparseMsg::tripletFill));

    static DestFinfo tripletFill1(
        "tripletFill1",
        ...
        new OpFunc1<SparseMsg, vector<unsigned int>>(&SparseMsg::tripletFill1));

    // Assemble it all.
    static Finfo* sparseMsgFinfos[] = {&numRows,         // readonly value
...
vector<unsigned int> SparseMsg::getEntryPairs() const
{
    ...
    for (unsigned int row = 0; row < matrix_.nRows(); ++row) {
        ...
        for (unsigned int j = begin; j < end; ++j) y.push_back(row);
    }
...
void SparseMsg::setRandomConnectivity(double probability, unsigned int seed)
{
    p_ = probability;
    rng_.setSeed(seed);
    randomConnect(probability);
...
void SparseMsg::setEntry(unsigned int row, unsigned int column,
                         unsigned int value)
{
...
void SparseMsg::pairFill(vector<unsigned int> src, vector<unsigned int> dest)
{
    // Sanity check
      ...
      seed_(-1)
{
    ...
    // cout << Shell::myNode() << ": SparseMsg constructor between " <<
    // e1->getName() << " and " << e2->getName() << endl;
...
Eref SparseMsg::firstTgt(const Eref& src) const
{
    ...
    if (src.element() == e1_) {
        ...
        const unsigned int* colIndex;
        unsigned int n =
            matrix_.getRow(src.dataIndex(), &fieldIndex, &colIndex);
        if (n != 0) {
...
Msg* SparseMsg::copy(Id origSrc, Id newSrc, Id newTgt, FuncId fid,
                     unsigned int b, unsigned int n) const
{
