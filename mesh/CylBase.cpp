...
CylBase::CylBase(
    double x, double y, double z, double dia, double length, unsigned int numDivs)
    : x_(x),
      y_(y),
      z_(z),
      dia_(dia),
...
double CylBase::voxelVolume(const CylBase& parent, unsigned int fid) const
{
    ...
    double frac0 = (static_cast<double>(fid)) / static_cast<double>(numDivs_);
    double frac1 = (static_cast<double>(fid + 1)) / static_cast<double>(numDivs_);
...
vector<double> CylBase::getCoordinates(const CylBase& parent, unsigned int fid) const
{
    assert(numDivs_ > fid);
    double frac0 = (static_cast<double>(fid)) / static_cast<double>(numDivs_);
    double frac1 = (static_cast<double>(fid + 1)) / static_cast<double>(numDivs_);
 ...
 */
double CylBase::getDiffusionArea(const CylBase& parent, unsigned int fid) const
{
...
double CylBase::getMiddleArea(const CylBase& parent, unsigned int fid) const
{
    assert(fid < numDivs_);
    if(isCylinder_)
        return M_PI * dia_ * dia_ / 4.0;
    double frac0 = (0.5 + static_cast<double>(fid)) / static_cast<double>(numDivs_);
    double r0    = 0.5 * (parent.dia_ * (1.0 - frac0) + dia_ * frac0);
    return M_PI * r0 * r0;
}
...
double CylBase::selectGridSize(double h, double dia1, double granularity) const
{
...
static void fillPointsOnCircle(const Vec& u, const Vec& v, const Vec& q, double h,
    double r, vector<double>& area, const CubeMesh* other)
{
...
static void fillPointsOnDisc(const Vec& u, const Vec& v, const Vec& q, double h, double r,
    vector<double>& area, const CubeMesh* other)
{
...
void CylBase::matchCubeMeshEntries(const ChemCompt* compt, const CylBase& parent,
    unsigned int startIndex, double granularity, vector<VoxelJunction>& ret,
    ...
    for(unsigned int i = 0; i < numDivs_; ++i) {
        ...
        if(useCylinderCurve) {
            for(unsigned int j = 0; j < num; ++j) {
                ...
                fillPointsOnCircle(u, v, Vec(q0, q1, q2), h, r, area, other);
            }
        }
        if(useCylinderCap && i == numDivs_ - 1) {
            fillPointsOnDisc(u, v, Vec(x_, y_, z_), h, dia_ / 2.0, area, other);
        }
...
double CylBase::nearest(
    double x, double y, double z, const CylBase& parent, double& linePos, double& r) const
{
...
double CylBase::nearest(
    double x, double y, double z, const CylBase& parent, unsigned int& index) const
{
    double k = 0.0;
    double r;
