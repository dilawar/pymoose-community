...
{
    static SrcFinfo0 finished("finished", "Signal for completion of run");
    return &finished;
}
...
{
    static vector<SrcFinfo1<ProcPtr>*> processVec = buildProcessVec("process");
    return processVec;
}

static vector<SrcFinfo1<ProcPtr>*>& reinitVec()
{
    static vector<SrcFinfo1<ProcPtr>*> reinitVec = buildProcessVec("reinit");
    return reinitVec;
}
...
const Cinfo* Clock::initCinfo()
{
    ...
    static ReadOnlyValueFinfo<Clock, double> runTime(
        "runTime", "Duration to run the simulation", &Clock::getRunTime);
    static ReadOnlyValueFinfo<Clock, double> currentTime(
        "currentTime", "Current simulation time", &Clock::getCurrentTime);
    static ReadOnlyValueFinfo<Clock, unsigned long> nsteps(
        "nsteps",
        "Number of steps to advance the simulation, in units of the smallest "
        "timestep on the clock ticks",
        &Clock::getNsteps);
    static ReadOnlyValueFinfo<Clock, unsigned int> numTicks(
        "numTicks", "Number of clock ticks", &Clock::getNumTicks);
    static ReadOnlyValueFinfo<Clock, unsigned int> stride(
        "stride",
        "Number by which the simulation advances the current step on each "
        "cycle. "
        "stride = smallest active timestep/smallest defined timestep.",
        &Clock::getStride);
    static ReadOnlyValueFinfo<Clock, unsigned long> currentStep(
        "currentStep", "Current simulation step", &Clock::getCurrentStep);

    static ReadOnlyValueFinfo<Clock, vector<double>> dts(
        "dts", "Utility function returning the dt (timestep) of all ticks.",
        &Clock::getDts);

    static ReadOnlyValueFinfo<Clock, bool> isRunning(
        "isRunning", "Utility function to report if simulation is in progress.",
        &Clock::isRunning);
    ...
    static LookupValueFinfo<Clock, unsigned int, unsigned int> tickStep(
        ...
        &Clock::setTickStep, &Clock::getTickStep);
    ...
    static LookupValueFinfo<Clock, unsigned int, double> tickDt(
        ...
        &Clock::setTickDt, &Clock::getTickDt);
    ...
    // MsgDest definitions
    ///////////////////////////////////////////////////////
    static DestFinfo start(
        "start", "Sets off the simulation for the specified duration",
        new EpFunc2<Clock, double, bool>(&Clock::handleStart));

    static DestFinfo step(
        "step",
    ...
    static DestFinfo stop(
        "stop", "Halts the simulation, with option to restart seamlessly",
        new OpFunc0<Clock>(&Clock::stop));

    static DestFinfo reinit("reinit", "Zeroes out all ticks, starts at t = 0",
    ...
    static SharedFinfo clockControl(
        "clockControl",
        "Controls all scheduling aspects of Clock, usually from Shell",
        clockControlFinfos, sizeof(clockControlFinfos) / sizeof(Finfo*));
    ...
    static Cinfo clockCinfo(
        ...
        Neutral::initCinfo(), clockFinfos, sizeof(clockFinfos) / sizeof(Finfo*),
...
{
    if(isRunning_) {
        cout << "Warning: Clock::setDt: Cannot change dt while simulation is "
                "running\n";
        return;
    }
    dt_ = v;
...
void Clock::setTickDt(unsigned int i, double v)
{
    ...
    if(v < minimumDt) {
        cout << "Warning: Clock::setTickDt: " << v
             << " is smaller than minimum allowed timestep " << minimumDt
             << endl;
        cout << "dt not set\n";
        return;
    }
...
void Clock::innerReportClock() const
{
    cout << "reporting Clock: runTime= " << runTime_
         << ", currentTime= " << currentTime_ << ", dt= " << dt_
         << ", isRunning = " << isRunning_ << endl;
    cout << "Dts= ";
    for(unsigned int i = 0; i < ticks_.size(); ++i) {
        cout << "tick[" << i << "] = " << ticks_[i] << "    " << ticks_[i] * dt_
             << endl;
    }
    cout << endl;
}
...
void Clock::handleStep(const Eref& e, unsigned long numSteps)
{
    numSteps *= stride_;
    if(isRunning_ || doingReinit_) {
        cout << "Clock::handleStart: Warning: simulation already in "
                "progress.\n Command ignored\n";
        return;
    }
    ...
    for(isRunning_ = (activeTicks_.size() > 0);
        ...
        for(vector<unsigned int>::iterator j = activeTicks_.begin();
            j != activeTicks_.end(); ++j) {
            if(endStep % *j == 0) {
                info_.dt = *j * dt_;
        ...
        // true.
        if(notify_) {
            if(isClose(fmod((100.0 * currentTime_) / runTime_, 10.0), 0.0,
                       1e-6)) {
                time(&rawtime);
                timeinfo = localtime(&rawtime);
                strftime(now, 80, "%c", timeinfo);
                cout << "@ " << now << ": " << 100 * currentTime_ / runTime_
                     << "% of total " << runTime_ << " seconds is over."
                     << endl;
            }
        }
...
{
    if(isRunning_ || doingReinit_) {
        cout << "Clock::handleReinit: Warning: simulation already in "
                "progress.\n Command ignored\n";
        return;
    }
    currentTime_ = 0.0;
...
{
    map<string, unsigned int>::const_iterator i = defaultTick_.find(className);
    if(i == defaultTick_.end()) {
        cout << "Warning: unknown className: '" << className << "'.\n"
             << "Advisable to update the defaultTick table in the Clock "
                "class.\n";
        return 0;
    }
    return i->second;
