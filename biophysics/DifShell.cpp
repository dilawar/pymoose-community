...
const Cinfo* DifShell::initCinfo()
{
    ...
    static string doc[] = {
        ...
        "DifShell",
        "Author",
        "Niraj Dudani. Ported to async13 by Subhasis Ray. Rewritten by Asia "
        "Jedrzejewska-Szmek",
        "Description",
        "DifShell object: Models diffusion of an ion (typically calcium) "
        "within an "
        "electric compartment. A DifShell is an iso-concentration region with "
        "respect to "
        "the ion. Adjoining DifShells exchange flux of this ion, and also keep "
        "track of "
        "changes in concentration due to pumping, buffering and channel "
        "currents, by "
        "talking to the appropriate objects.",
    };
    static Dinfo<DifShell> dinfo;
    static Cinfo difShellCinfo("DifShell", DifShellBase::initCinfo(), 0, 0,
...
{
    if(shapeMode_ != 3)
        cerr << "Warning: DifShell: Trying to set volume, when shapeMode is "
                "not USER-DEFINED\n";
...
{
    if(shapeMode_ != 3)
        cerr << "Warning: DifShell: Trying to set outerArea, when shapeMode is "
                "not USER-DEFINED\n";
...
{
    if(shapeMode_ != 3)
        cerr << "Warning: DifShell: Trying to set innerArea, when shapeMode is "
                "not USER-DEFINED\n";
    ...
    switch(shapeMode_) {
        ...
        case 0:
            if(length_ == 0.0) {  // Spherical shell
                volume_ =
                    4. / 3. * M_PI * (rOut * rOut * rOut - rIn * rIn * rIn);
        ...
        case 1:
             ...
             */
        case 3:
            // Nothing to be done here. Volume and inner-, outer areas specified
            // by user.
            break;
...
void DifShell::vProcess(const Eref& e, ProcPtr p)
{
     ...
     * the reaction (forward / backward rates ; free / bound buffer
     * concentration) immediately, which this DifShell will use to find amount
     * of ion captured or released in the current time-step.
     */
...
void DifShell::vBuffer(const Eref& e, double kf, double kb, double bFree,
                       double bBound)
{
    dCbyDt_ += kb * bBound;
...
{
    double diff =
        2. * D_ / volume_ * outerArea_ / (outerThickness + thickness_);
...
void DifShell::vFluxFromIn(const Eref& e, double innerC, double innerThickness)
{
    ...
    double diff =
        2. * D_ / volume_ * innerArea_ / (innerThickness + thickness_);
...
void DifShell::vHillPump(const Eref& e, double vMax, double Kd,
                         unsigned int hill)
{
    // This needs fixing
    double ch;
