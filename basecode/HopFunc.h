...
public:
    HopFunc0(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
...
template <class A>
class HopFunc1 : public OpFunc1Base<A> {
public:
    HopFunc1(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
    ...
    unsigned int localOpVec(Element* elm, const vector<A>& arg,
                            const OpFunc1Base<A>* op, unsigned int k) const
    {
    ...
    unsigned int localFieldOpVec(const Eref& er, const vector<A>& arg,
    ...
    unsigned int remoteOpVec(const Eref& er, const vector<A>& arg,
                             const OpFunc1Base<A>* op, unsigned int start,
                             unsigned int end) const
    {
        ...
        if (mooseNumNodes() > 1 && nn > 0) {
            ...
            double* buf = addToBuf(er, hopIndex_, Conv<vector<A>>::size(temp));
...
class HopFunc2 : public OpFunc2Base<A1, A2> {
    using OpFunc2Base<A1, A2>::opVec;

public:
    HopFunc2(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
    ...
    {
        double* buf =
            addToBuf(e, hopIndex_, Conv<A1>::size(arg1) + Conv<A2>::size(arg2));
        /*
    ...
    void opVec(const Eref& e, const vector<A1>& arg1, const vector<A1>& arg2,
               const OpFunc2Base<A1, A2>* op) const
    {
        ...
        for (unsigned int i = 0; i < mooseNumNodes(); ++i) {
            if (i == mooseMyNode()) {
                ...
                double* buf =
                    addToBuf(e, hopIndex_, Conv<vector<A1>>::size(temp1) +
...
class HopFunc3 : public OpFunc3Base<A1, A2, A3> {
public:
    HopFunc3(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
...
class HopFunc4 : public OpFunc4Base<A1, A2, A3, A4> {
public:
    HopFunc4(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
...
public:
    HopFunc5(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
...
template <class A1, class A2, class A3, class A4, class A5>
const OpFunc* OpFunc5Base<A1, A2, A3, A4, A5>::makeHopFunc(HopIndex hopIndex)
    const
{
...
public:
    HopFunc6(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }

    void op(const Eref& e, A1 arg1, A2 arg2, A3 arg3, A4 arg4, A5 arg5,
            A6 arg6) const
    {
        double* buf = addToBuf(e, hopIndex_,
                               Conv<A1>::size(arg1) + Conv<A2>::size(arg2) +
...
template <class A>
class GetHopFunc : public OpFunc1Base<A*> {
public:
    GetHopFunc(HopIndex hopIndex) : hopIndex_(hopIndex)
    {
        ;
    }
                          ...
                          const GetOpFuncBase<A>* op) const
    {
        ...
        Element* elm = er.element();
        unsigned int numField = elm->numField(p - elm->localDataStart());
        for (unsigned int q = 0; q < numField; ++q) {
            Eref temp(elm, p, q);
                         ...
                         const GetOpFuncBase<A>* op) const
    {
        ...
        for (unsigned int i = 0; i < mooseNumNodes(); ++i) {
            if (i == mooseMyNode()) {
                ...
                assert(static_cast<unsigned int>(temp[0]) == numOnNode[i]);
