...
A localGet(const Eref& er, string field)
{
    ...
    const GetOpFunc<T, A>* gop = dynamic_cast<const GetOpFunc<T, A>*>(op);
    assert(gop);
...
public:
     ...
     */
    static bool strSet(const ObjId& dest, const string& field,
                       const string& val);

    /// Sends out request for data, and awaits its return.
    static const vector<double*>* dispatchGet(const ObjId& tgt, FuncId tgtFid,
                                              const double* arg,
                                              unsigned int size);
...
public:
    ...
    static bool set(const ObjId& dest, const string& field)
    {
        ...
        const OpFunc0Base* op = dynamic_cast<const OpFunc0Base*>(func);
        if (op) {
            if (tgt.isOffNode()) {
                const OpFunc* op2 =
                    op->makeHopFunc(HopIndex(op->opIndex(), MooseSetHop));
                const OpFunc0Base* hop = dynamic_cast<const OpFunc0Base*>(op2);
                assert(hop);
                hop->op(tgt.eref());
                delete op2;
                if (tgt.isGlobal())
...
public:
    ...
    static bool setVec(ObjId destId, const string& field, const vector<A>& arg)
    {
        ...
        if (op) {
            const OpFunc* op2 =
                op->makeHopFunc(HopIndex(op->opIndex(), MooseSetVecHop));
            const OpFunc1Base<A>* hop =
                dynamic_cast<const OpFunc1Base<A>*>(op2);
...
public:
    ...
    static A get(const ObjId& dest, const string& field)
    {
        ...
        if (gof) {
            if (tgt.isDataHere()) {
                ...
                const OpFunc* op2 =
                    gof->makeHopFunc(HopIndex(gof->opIndex(), MooseGetHop));
                const OpFunc1Base<A*>* hop =
                    dynamic_cast<const OpFunc1Base<A*>*>(op2);
                assert(hop);
                // Blocking function.
    ...
    static void getVec(ObjId dest, const string& field, vector<A>& vec)
    {
        ...
        if (gof) {
            const OpFunc* op2 =
                gof->makeHopFunc(HopIndex(gof->opIndex(), MooseGetVecHop));
            const GetHopFunc<A>* hop = dynamic_cast<const GetHopFunc<A>*>(op2);
            hop->opGetVec(tgt.eref(), vec, gof);
            delete op2;
            return;
        }
        cout << "Warning: Field::getVec conversion error for " << dest.path()
             << endl;
    }
...
public:
     ...
     */
    static bool setVec(Id destId, const string& field, const vector<A1>& arg1,
...
class LookupField : public SetGet2<L, A> {
public:
    LookupField(const ObjId& dest) : SetGet2<L, A>(dest)
    {
        ;
    }
     ...
     */
    static bool setVec(Id destId, const string& field, const vector<L>& index,
                       const vector<A>& arg)
    {
        string temp = "set" + field;
        temp[3] = std::toupper(temp[3]);
     ...
     */
    static bool setVec(ObjId dest, const string& field, const vector<L>& index,
                       const vector<A>& arg)
    {
        string temp = "set" + field;
        temp[3] = std::toupper(temp[3]);
     ...
     */
    static void getVec(Id dest, const string& field, vector<L>& index,
                       vector<A>& vec)
    {
        vec.resize(0);
        ObjId tgt(dest);
        ...
        if (op) {
            if (tgt.isOffNode()) {
                ...
                const OpFunc3Base<A1, A2, A3>* hop =
                    dynamic_cast<const OpFunc3Base<A1, A2, A3>*>(op2);
...
public:
     ...
     */
    static bool set(const ObjId& dest, const string& field, A1 arg1, A2 arg2,
                    A3 arg3, A4 arg4, A5 arg5, A6 arg6)
    {
        FuncId fid;
        ObjId tgt(dest);
        const OpFunc* func = checkSet(field, tgt, fid);
        ...
        if (op) {
            if (tgt.isOffNode()) {
                ...
                const OpFunc6Base<A1, A2, A3, A4, A5, A6>* hop =
                    dynamic_cast<const OpFunc6Base<A1, A2, A3, A4, A5, A6>*>(
                        op2);
