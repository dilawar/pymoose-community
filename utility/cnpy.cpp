...
void changeHeaderShape(std::fstream& fs, const size_t data_len, const size_t numcols)
{
    ...
    vector<string> tokens = split(shapeStr, ",");
...
size_t writeHeader(
    std::fstream& fs, const vector<string>& colnames, const vector<size_t>& shape)
{
...
{
    std::fstream fs;
    fs.open(outfile, std::fstream::in | std::fstream::out | std::fstream::trunc |
                         std::ofstream::binary);
...
void writeNumpy(
    const string& outfile, const vector<double>& data, const vector<string>& colnames)
{
    ...
    // Create a new file.
    std::fstream fs;
    fs.open(outfile, std::ofstream::in | std::ofstream::out | std::ofstream::binary |
                         std::ofstream::trunc);
...
void appendNumpy(
    const string& outfile, const vector<double>& vec, const vector<string>& colnames)
{
